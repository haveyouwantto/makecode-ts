...
namespace custom {
     ...
     */
    //% block
    export function mandelbrot(x: number, y: number, magn: number): void {
        ...
        while (ya < 2 / magn + ym) {
            ...
            if (I != maxiter) {
                blocks.place(palette[I % 29], positions.createWorld(posx + 1 + (xa + 2 / magn - xm) * sizex * magn, posy + I, posz + 1 + (ya + 2 / magn - ym) * sizey * magn))
            } else {
